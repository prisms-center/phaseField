  ...
  , pp_attributes(_pp_attributes)
{
  ...
  const uint num_constants = model_constant_names.size();
                                           ...
                                           const std::string &default_entry)
{
  ...
  for (uint i = 0; i < entry_list.size(); i++)
    {
      for (uint j = 0; j < entry_list.size(); j++)
        {
          if (i == j)
            {
      ...
      if (variable.pde_type == TIME_INDEPENDENT ||
          variable.pde_type == IMPLICIT_TIME_DEPENDENT)
        {
          ...
          parameter_handler.enter_subsection(subsection_text);
          {
            ...
            parameter_handler.declare_entry("Smoother iterations",
                                            "5",
                                            dealii::Patterns::Integer(0, INT_MAX),
