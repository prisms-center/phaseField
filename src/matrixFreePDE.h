...
class MatrixFreePDE:public Subscriptor
{
 public:
  MatrixFreePDE(); 
  void init (std::vector<Field>& _fields);
  void run ();
 ...
 private:
  ...
  std::vector<const IndexSet*>         locally_relevant_dofs;
...
template <int dim>
MatrixFreePDE<dim>::init(std::vector<Field>& _fields){
  //store pointer to fields vector. Assuming vector will be persistent
  //in memory.
  fields=&_fields; 
...
void MatrixFreePDE<dim>::updateRHS (){
  ...
  data.cell_loop (&MatrixFreePDE::computeRHS, this, residualU, U);
...
void MatrixFreePDE<dim>::vmult (vectorType &dst, const vectorType &src) const{
  dst=0.0;
  data.cell_loop (&MatrixFreePDE::computeRHS, this, dst, src);
...
template <int dim>
void MatrixFreePDE<dim>::setup_matrixfree (){
...
//setup
template <int dim>
void MatrixFreePDE<dim>::setup_system ()
{
  Timer time;
  time.start ();
  setup_time = 0;
...
//output 
template <int dim>
void MatrixFreePDE<dim>::output_results (const unsigned int cycle)
{
