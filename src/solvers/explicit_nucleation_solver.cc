...
#include <prismspf/solvers/explicit_nucleation_solver.h>
...
ExplicitNucleationSolver<dim, degree>::ExplicitNucleationSolver(
...
template <unsigned int dim, unsigned int degree>
void
ExplicitNucleationSolver<dim, degree>::init()
{
  this->compute_subset_attributes(FieldSolveType::ExplicitNucleation);
...
template <unsigned int dim, unsigned int degree>
void
ExplicitNucleationSolver<dim, degree>::solve()
{
  // If the subset attribute is empty return early
  if (this->get_subset_attributes().empty())
    {
  ...
  // Compute the nucleation probability fields
  this->get_system_matrix()->compute_nucleation_explicit_update(new_solution_subset,
                                                                solution_subset);
  ...
  this->get_solution_handler().update(FieldSolveType::ExplicitNucleation);

  // Update the ghosts
  this->get_solution_handler().update_ghosts();
}

INSTANTIATE_BI_TEMPLATE(ExplicitNucleationSolver)
