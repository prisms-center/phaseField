                ...
                if (g_other != g_base) {
                    ...
                    if ((sum_radii + 2.0 * buffer_distance > center_distance) and (order_parameter_other == order_parameter_base)) {
                        ...
                        if (overlap_counter >= cycle) {
                            ...
                            if (dealii::Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0) {
                                std::cout << "Reassigning grain " << grain_representations.at(g_base).getGrainId() << " from order parameter " << grain_representations.at(g_base).getOldOrderParameterId() << " to order parameter " << grain_representations.at(g_base).getOrderParameterId() << std::endl
                                          << std::endl;
                            }
                        }
                    }
