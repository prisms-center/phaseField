					  ...
					  const std::pair<unsigned int,unsigned int> &cell_range) const
{
  ...
  for (unsigned int i=0; i<numStructuralOrderParameters+2; i++){
  ...
  constCx=-Mc*dt; constNx=-Mn[0]*Kn[0]*dt;
  //loop over all "cells"
  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell){
    ...
    for (unsigned int q=0; q<vals[0]->n_q_points; ++q){
      VectorizedArray<double> c  = vals[0]->get_value(q), mu  = vals[1]->get_value(q), n[numStructuralOrderParameters];
      Tensor<1, dim, VectorizedArray<double> >  cx = vals[0]->get_gradient(q), mux = vals[1]->get_gradient(q), nx[numStructuralOrderParameters];
      for (unsigned int i=0; i<numStructuralOrderParameters; i++){
	n[i]=vals[i+2]->get_value(q);
	nx[i]=vals[i+2]->get_gradient(q);
      }
      for (unsigned int i=0; i<numStructuralOrderParameters+2; i++){
	if (i==0) {
	  vals[i]->submit_value(rcV,q);
	  vals[i]->submit_gradient(constCx*rcxV,q);
	}
	else if (i==1) vals[i]->submit_value(rmuV,q); 
	else{
	  vals[i]->submit_value(rnV,q);
	  vals[i]->submit_gradient(constNx*rnxV,q);
	}
      }       
    }
    for (unsigned int i=0; i<numStructuralOrderParameters+2; i++){
      if (i==0) vals[i]->integrate(true,true);
      if (i==1) vals[i]->integrate(true,false);
...
void PrecipitateProblem<dim>::setup_system ()
{
  ...
  residualC.reinit(C);
  Mu.reinit(C); residualMu.reinit(C);
  for (unsigned int i=0; i<numStructuralOrderParameters; i++){
  ...
  for (unsigned int j=0; j<Mu.local_size(); ++j){
    double c, n[1]; 
    c=C.local_element(j); n[0]=N[0].local_element(j);
    Mu.local_element(j)=rmuV;
  } 
  //timing
  setup_time += time.wall_time();
...
void PrecipitateProblem<dim>::solve ()
{
  ...
  for (unsigned int i=0; i<numStructuralOrderParameters; i++){
    for (unsigned int j=0; j<N[0].local_size(); ++j)
      if (increment>100) N[i].local_element(j)=invM.local_element(j)*residualN[i].local_element(j);
  }
