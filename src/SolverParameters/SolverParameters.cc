...
unsigned int SolverParametersBase::getEquationIndex(unsigned int global_index){
    ...
    std::cerr << "PRISMS-PF Error: Attempted access of a parameter for the nonlinear solver for an ineligible variable index." << std::endl;
    abort();
}
...
double NonlinearSolverParameters::getMaxIterations(){
