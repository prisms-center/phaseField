...
{
  // We have to do this recurively to get all possible combinations. If the list is empty,
  // dump the text. Other perform the substitutions one at a time
...
read_templates(const std::string &filename)
{
  ...
  // Throw an error if the file extension is wrong
  if (!filename.ends_with(".in"))
    {
      std::cerr << "Template file " << filename << " has the wrong extension\n"
                << std::flush;
      std::exit(1);
    }
  ...
  while (!class_template_in.empty())
    {
      ...
      if (!has_prefix(class_template_in, "for"))
        {
          std::cerr << "Invalid template instantation list. Missing `for`\n"
                    << std::flush;
          std::exit(1);
        }
      ...
      if (!has_prefix(class_template_in, "("))
        {
          std::cerr << "Invalid template instantation list. Missing `(`\n" << std::flush;
          std::exit(1);
        }
      ...
      for (const auto &expansion_key : expansion_keys)
        {
          ...
          if (value_key.size() != 2)
            {
              std::cerr << "Invalid template instantation list. Expansions should be "
      ...
      // Grab the explicit template instantations we have to substite
