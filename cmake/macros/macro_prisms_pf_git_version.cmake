...
macro(prisms_pf_git_version)
  ...
  if(GIT_FOUND AND EXISTS ${CMAKE_SOURCE_DIR}/.git/HEAD)
    ...
    execute_process(
      COMMAND git log -n 1 --pretty=format:"revision=%H, shortrev=%h, date=%cd" ${date_formatting}
      WORKING_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}
      OUTPUT_VARIABLE tmp2
      OUTPUT_STRIP_TRAILING_WHITESPACE
    )
    if(NOT ${tmp2} STREQUAL "")
      string(REGEX REPLACE "^\"revision=(.+), shortrev=(.+), date=(.+)\"$" "\\1" PRISMS_PF_GIT_REVISION "${tmp2}")
      string(REGEX REPLACE "^\"revision=(.+), shortrev=(.+), date=(.+)\"$" "\\2" PRISMS_PF_GIT_SHORTREV "${tmp2}")
      string(REGEX REPLACE "^\"revision=(.+), shortrev=(.+), date=(.+)\"$" "\\3" PRISMS_PF_GIT_TIMESTAMP "${tmp2}")
