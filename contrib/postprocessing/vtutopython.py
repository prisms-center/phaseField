...
def vtutopython(filename, scalars=[], vectors=[], dim=2):
    ...
    if dim == 2:
        ...
        for field in vectors:
            ...
            vecindices.append(
                (indexcount, field)
            )  # we will put the fields in a numpy array, so need to know what the indices will be
            indexcount += 2
        ...
        df = (
            df.drop_duplicates()
        )  # all values are repeated for each cell node, need to get rid of them
        df = df.sort_values(
            ["x", "y"], ascending=[True, True]
        )  # sort by position so that we can easily reshape the array
        cleandata = (
            df.to_numpy()
        )  # we need to put things in a numpy array so they can be reshaped
        ...
        xdiv = (
            round(xsize / dh) + 1
        )  # number of points in each direction (number of cells + 1)
        ...
        for i in vecindices:
            outputs[i[1]] = np.array(
                [
                    cleandata[:, i[0]].reshape(xdiv, ydiv),
                    cleandata[:, i[0] + 1].reshape(xdiv, ydiv),
                ]
            )
        ...
        for i in vecindices:
            outputs[i[1]] = np.array(
                [
                    cleandata[:, i[0]].reshape(xdiv, ydiv, zdiv),
                    cleandata[:, i[0] + 1].reshape(xdiv, ydiv, zdiv),
                    cleandata[:, i[0] + 2].reshape(xdiv, ydiv, zdiv),
                ]
            )
...
with open(outfile, "wb") as f:
    pickle.dump(
        vtutopython(filename=infile, scalars=scalars, vectors=vectors, dim=ndim), f
    )
