...
def create_pvtu_file(base_name, split_filenames, reference_mesh):
    with open(f"{base_name}.pvtu", "w") as file:
        ...
        for array in reference_mesh.point_data.keys():
            num_components = (
                reference_mesh.point_data[array].shape[1]
                if len(reference_mesh.point_data[array].shape) > 1
                else 1
            )
            data_type = pv.get_array_association(reference_mesh, array)
            file.write(
                f'    <PDataArray type="{data_type}" Name="{array}" NumberOfComponents="{num_components}"/>\n'
            )
...
try:
    ...
    print(
        "Error: A single integer argument must be provided specifying the number files in which the system will be split"
    )
