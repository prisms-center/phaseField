...
TEST_CASE("Read vtk file")
{
  SECTION("2D Scalar fields with quadrature degree 1")
  {
    ...
    for (double x_coord : {0, 5, 10})
      {
        for (double y_coord : {0, 5, 10})
          {
            double    point[3] = {x_coord, y_coord, 0};
            vtkIdType point_id = output->FindPoint(point);
          }
      }
  }
