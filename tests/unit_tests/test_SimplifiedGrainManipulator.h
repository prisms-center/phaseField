...
template <int dim,typename T>
  bool unitTest<dim,T>::test_SimplifiedGrainManipulator_transferGrainIds(){
    ...
    // Subtest 1 (two grains both needing reassignment)
    {
        ...
        std::vector<SimplifiedGrainRepresentation<dim>> old_grain_representations, new_grain_representations;
        ...
        std::cout << "Grain ids before transfer: " << new_grain_representations.at(0).getGrainId() << " " << new_grain_representations.at(1).getGrainId() << std::endl;

        SimplifiedGrainManipulator<dim> simplified_grain_manipulator;
        simplified_grain_manipulator.transferGrainIds(old_grain_representations, new_grain_representations);
...
template <int dim,typename T>
  bool unitTest<dim,T>::test_SimplifiedGrainManipulator_reassignGrains(){
    ...
    // Subtest 1 (three grains, two order parameters, one grain needs reassignment)
    {
        ...
        for (unsigned int g=0; g < grain_representations.size(); g++){
            std::cout << "Grain ops before reassignment: " << grain_representations.at(g).getOrderParameterId() << std::endl;
        }
        */
        SimplifiedGrainManipulator<dim> simplified_grain_manipulator;
        simplified_grain_manipulator.reassignGrains(grain_representations, 0.5, order_parameter_id_list);
        /*
        for (unsigned int g=0; g < grain_representations.size(); g++){
            std::cout << "Grain ops after reassignment: " << grain_representations.at(g).getOrderParameterId() << std::endl;
        }
        */
        bool result = false;
