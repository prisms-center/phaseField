...
# ----------------------------------------------------------------------------------------
def run_simulation(refineFactor,degree,subdivisionsX,subdivisionsY,subdivisionsZ,timeIncrements,timeStep,run_name):
	
	timeFinal = 100000000
	skipImplicitSolves = 10000000
	
	text_file = open("time_and_space_parameters.h","w")
	text_file.write("// Parameters list for the parameters involved in mesh generation and time stepping \n") 
...
def getError(run_num,maxIter):
	...
	text_file = open("output_num_1.txt","w")
	text_file.write(maxIter) 
	text_file.close() 
...
run_simulation(4,1,1,1,1,1600,5e-4,'run_001') # Highest stable dt for this order and mesh size
run_simulation(0,2,5,5,5,1600,5e-4,'run_002')
run_simulation(0,2,5,5,5,800,1.0e-3,'run_003')
run_simulation(0,2,5,5,5,850,0.8/850.0,'run_004') # Highest stable dt for this order and mesh size
run_simulation(4,1,1,1,1,100000,8.0e-6,'run_005')
run_simulation(0,2,5,5,5,100000,8.0e-6,'run_006')
if re_run_ref_simulation == True:
	run_simulation(7,1,1,1,1,100000,8.0e-6,'reference_solution')
...
text_file = open("output_num_2.txt","w")
text_file.write('100000') 
text_file.close() 	
	


error = []
error.append(getError(1,'01600'))
error.append(getError(2,'01600'))
error.append(getError(3,'0800'))
error.append(getError(4,'0850'))
error.append(getError(5,'100000'))
error.append(getError(6,'100000'))

# Calculate and output the error

print "Error results: \n"
print str(error[0]), str(error[1]), str(error[2]), str(error[3]), str(error[4]), str(error[5])
