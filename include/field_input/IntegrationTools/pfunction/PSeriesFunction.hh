...
namespace PRISMS
{
  ...
  ///
  /// Example template parameters are: <double, double, std::vector<double>,
  /// std::vector<int> >
  ///
  ...
  public:
    ...
    PSeriesFunction(OutType                                        zero,
                    OutType                                        identity,
                    const std::vector<PBasisSet<InType, OutType>> &basis_set)
    {
      _zero_val     = zero;
      _identity_val = identity;
    ...
    /// generate the PSeriesFunction, by cloning 'basis_set' and resizing everything to
    /// match
    ///
    void
    set(const std::vector<PBasisSet<InType, OutType>> &basis_set)
    {
    ...
    virtual PSeriesFunction<InType, OutType, VarContainer, IndexContainer> *
    clone() const
    {
    ...
    virtual OutType
    basis(int bindex, int term, const VarContainer &var)
    {
      return _basis_set[bindex](term, var[bindex]);
    }

    virtual OutType
    basis_grad(int bindex, int term, const VarContainer &var)
    {
      return _basis_set[bindex].grad(term, var[bindex]);
    }

    virtual OutType
    basis_hess(int bindex, int term, const VarContainer &var)
    {
    ...
    virtual OutType
    basis(const IndexContainer &term, const VarContainer &var)
    {
      // evaluate basis functions needed
    ...
    virtual OutType
    basis_grad(const IndexContainer &term, const VarContainer &var, int di)
    {
      // evaluate basis functions needed
      OutType tmp = _identity_val;
      for (int i = 0; i < _coeff.order(); i++)
    ...
    virtual OutType
    basis_hess(const IndexContainer &term, const VarContainer &var, int di, int dj)
    {
      // evaluate basis functions needed
      OutType tmp = _identity_val;
      for (int i = 0; i < _coeff.order(); i++)
    ...
    virtual void
    eval_basis(const VarContainer &var)
    {
    ...
    virtual void
    eval_basis(const VarContainer &var, int i)
    {
      // evaluate basis functions
      _basis_set[i].eval(var[i]);
    }
    ...
    virtual void
    eval_basis_grad(const VarContainer &var)
    {
    ...
    virtual void
    eval_basis_grad(const VarContainer &var, int i)
    {
      // evaluate basis grad functions
      _basis_set[i].eval_grad(var[i]);
    }
    ...
    virtual void
    eval_basis_hess(const VarContainer &var)
    {
    ...
    virtual void
    eval_basis_hess(const VarContainer &var, int i)
    {
      // evaluate basis hess functions
      _basis_set[i].eval_hess(var[i]);
    }
    ...
    // use basis index and term index for individual basis function
    virtual OutType
    basis(int bindex, int term) const
    {
      return _basis_set[bindex](term);
    }

    virtual OutType
    basis_grad(int bindex, int term) const
    {
      return _basis_set[bindex].grad(term);
    }

    virtual OutType
    basis_hess(int bindex, int term) const
    {
    ...
    // or use tensor indices to evaluate basis function product
    virtual OutType
    basis(const IndexContainer &term) const
    {
    ...
    virtual OutType
    basis_grad(const IndexContainer &term, int di) const
    {
      // evaluate basis function product
      OutType tmp = _identity_val;
      for (int i = 0; i < _coeff.order(); i++)
    ...
    virtual OutType
    basis_hess(const IndexContainer &term, int di, int dj) const
    {
      // evaluate basis function product
      OutType tmp = _identity_val;
      for (int i = 0; i < _coeff.order(); i++)
    ...
    void
    print_basis_grad(std::ostream &sout, int di)
    {
      std::vector<int> term = std::vector<int>(_coeff.order(), 0);
      for (int i = 0; i < _coeff.volume(); i++)
        {
    ...
    void
    print_basis_hess(std::ostream &sout, int di, int dj)
    {
      std::vector<int> term = std::vector<int>(_coeff.order(), 0);
      for (int i = 0; i < _coeff.volume(); i++)
        {
  ...
  private:
    // ----------------------------------------------------------
    // These assume you've evaluated the necessary _basis, _basis_grad, and _basis_hess
    // functions
