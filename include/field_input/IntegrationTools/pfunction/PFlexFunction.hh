...
namespace PRISMS
{
  ...
  public:
    ...
    PFlexFunction(const std::string                            &name,
                  ...
                  const std::vector<std::vector<PSimpleFunction<VarContainer, OutType>>>
                    &hess_simplef)
    ...
    PFlexFunction &
    operator=(const PFlexFunction &RHS)
    {
      this->_name            = RHS._name;
      this->_var_name        = RHS._var_name;
      this->_var_description = RHS._var_description;
    ...
    PFlexFunction<VarContainer, OutType> *
    clone() const
    {
  ...
  private:
    ...
    check()
    {
      ...
      if (this->_var_description.size() != n)
        {
          std::cerr
            << "Error in PFlexFunction. _var_name.size() != _var_description.size()."
            << std::endl;
          exit(1);
        }
      if (_grad_val.size() != n)
        {
          std::cerr << "Error in PFlexFunction. _var_name.size() != _grad_val.size()."
                    << std::endl;
          exit(1);
        }
      if (_hess_val.size() != n)
        {
          std::cerr << "Error in PFlexFunction. _var_name.size() != _hess_val.size()."
                    << std::endl;
          exit(1);
        }
      for (size_type i = 0; i < _hess_val.size(); i++)
        {
          if (_hess_val[i].size() != n)
            {
              std::cerr << "Error in PFlexFunction. _var_name.size() != _hess_val[" << i
                        << "].size()." << std::endl;
              exit(1);
            }
        }
