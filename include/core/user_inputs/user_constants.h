...
private:
   ...
   */
  uint
  compute_tensor_parentheses(const uint                      n_elements,
   ...
   */
  dealii::Tensor<1, dim>
  compute_rank_1_tensor_constant(const uint               n_elements,
   ...
   */
  dealii::Tensor<2, dim>
  compute_rank_2_tensor_constant(const uint               n_elements,
                                 std::vector<std::string> tensor_elements);
...
userConstants<dim>::compute_tensor_parentheses(
  ...
  const std::vector<std::string> &tensor_elements)
{
  uint open_parentheses  = 0;
  uint close_parentheses = 0;

  for (uint element = 0; element < n_elements; element++)
    {
  ...
  if (model_constants_strings.size() == 2)
  ...
  else
    {
      if (boost::iequals(model_constants_type_strings.at(0), "tensor"))
        {
          const uint n_elements = model_constants_strings.size() - 1;

          const uint open_parentheses =
  ...
  if ((mat_model == ANISOTROPIC) && (dim == 2) && elastic_constants.size() == 21)
    {
      ...
      const std::vector<uint> indices_2D = {0, 1, 5, 6, 10, 14};
                                 ...
                                 const std::vector<double> &constants) const
{
  ...
  constexpr uint voight_matrix_size = (2 * dim) - 1 + (dim / 3);
  for (uint i = 0; i < voight_matrix_size; i++)
    {
      for (uint j = 0; j < voight_matrix_size; j++)
        {
