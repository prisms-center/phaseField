...
// Note: could make NucleationHandler a namespace as everything is static
  ...
  for (const auto &[index, variable] : user_inputs.get_variable_attributes())
    {
      ...
      for (const auto &cell : solver_context.get_triangulation_handler()
                                ...
                                .active_cell_iterators())
        {
          ...
          fe_values.get_function_values(*(solver_context.get_solution_handler()
          ...
          for (unsigned int i = 0; i < num_nuclei_in_cell; ++i)
            {
              ...
              unsigned int nucleating_index =
                variable.get_nucleating_field_indices()[nucleating_index_dist(rng)];
...
template <unsigned int dim, unsigned int degree, typename number>
inline bool
NucleationHandler<dim, degree, number>::gather_exclude_broadcast_nuclei(
  std::list<Nucleus<dim>>        &new_nuclei_list,
  std::vector<Nucleus<dim>>      &global_nuclei,
  const UserInputParameters<dim> &user_inputs)
{
