...
void customPDE<dim,degree>::explicitEquationRHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
    ...
    scalarvalueType irxn = constV(i0V)*(constV(1.0)-cM/constV(cMsatV))*std::exp(constV(zMV*(1.0-betaV)*FarC/(RV*TV))*eta);
...
void customPDE<dim,degree>::nonExplicitEquationRHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
 ...
 // --- Getting the values and derivatives of the model variables ---
    ...
    //Temporary laplace equation
    //scalarvalueType rPhi = constV(0.0);
    //scalargradType rPhix = -Phix;
    
    // Residuals for the equation to evolve the order parameter
    variable_list.set_scalar_value_term_RHS(5,rPhi);
    variable_list.set_scalar_gradient_term_RHS(5,rPhix);
...
void customPDE<dim,degree>::equationLHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
    ...
    scalarvalueType DPhi = variable_list.get_change_in_scalar_value(5);
    scalargradType DPhix = variable_list.get_change_in_scalar_gradient(5);
    ...
    //Derivative of irxn with respect to Phi
    //scalarvalueType irxnp1=constV(i0V)*constV(zMV*betaV*FarC/(RV*TV))*std::exp(constV(-zMV*betaV*FarC/(RV*TV))*eta);
    //scalarvalueType irxnp2=-constV(i0V)*constV(zMV*(1.0-betaV)*FarC/(RV*TV))*(constV(1.0)-cM/constV(cMsatV))*std::exp(constV(zMV*(1.0-betaV)*FarC/(RV*TV))*eta);
    //scalarvalueType irxnp=irxnp1+irxnp2;
    scalarvalueType irxnp = - constV(i0V)*(constV(1.0)-cM/constV(cMsatV))*constV(zMV*(1.0-betaV)*FarC/(RV*TV))*std::exp(constV(zMV*(1.0-betaV)*FarC/(RV*TV))*eta);
    
    scalarvalueType rPhi = magpsix*irxnp*DPhi;
    scalargradType rPhix = -psi*kappa*DPhix;
    //Temporary laplace equation
    //scalarvalueType rPhi = constV(0.0);
    //scalargradType rPhix = DPhix;
    
    // Residuals for the equation to evolve the order parameter
    variable_list.set_scalar_value_term_LHS(5,rPhi);
    variable_list.set_scalar_gradient_term_LHS(5,rPhix);
