...
public:
  ...
  static constexpr double c1      = 0.04;
  static constexpr double c0      = 0.5;

  static constexpr double k       = 0.09;
  static constexpr double epsilon = 90.0;
...
private:
    ...
    if (index == 0) // c
      {
        ...
        scalar_value =
          c0 + c1 * (cos(0.2 * x) * cos(0.11 * y) + squared_term * squared_term +
              ...
              [[maybe_unused]] unsigned int solve_block_id) const override
  {
    ...
    else if (solve_block_id == 3) // Phi
      {
        ScalarValue c_val = variable_list.template get_value<Scalar, Current>(0);
        variable_list.set_value_term(3, -k * c_val / epsilon);
      }
    else if (solve_block_id == 2) // pp
      {
        ScalarValue c_val   = variable_list.template get_value<Scalar, Current>(0);
        ScalarGrad  c_grad  = variable_list.template get_gradient<Scalar, Current>(0);
        ScalarValue Phi_val = variable_list.template get_value<Scalar, Current>(3);
        ...
        ScalarValue f_electro  = 0.5 * k * c_val * Phi_val;
        ScalarValue f_tot      = f_chemical + f_gradient + f_electro;
        variable_list.set_value_term(2, f_tot);
      }
  }

  void
  compute_lhs([[maybe_unused]] FieldContainer<2, degree, number> &variable_list,
              [[maybe_unused]] const SimulationTimer             &sim_timer,
              [[maybe_unused]] unsigned int solve_block_id) const override
  {
    if (solve_block_id == 3) // Phi
      {
        ScalarGrad Phi_grad = variable_list.template get_gradient<Scalar, LHS>(3);
        variable_list.set_gradient_term(3, -Phi_grad);
      }
  }
};
