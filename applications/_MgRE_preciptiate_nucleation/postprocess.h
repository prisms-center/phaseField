...
#define pp_variable_name {"f_el","mu_c","mu_n1","mu_n2","mu_n3","component"}
#define pp_variable_type {"SCALAR","SCALAR","SCALAR","SCALAR","SCALAR","SCALAR"}
...
#define pp_need_val_residual {true,true,true,true,true,true}
#define pp_need_grad_residual {false,false,true,true,true,false}
...
if (n_dependent_stiffness == true){
	...
	heterMechAC2 *= 0.5*hn3V;
}


scalarvalueType mu_n1, mu_n2, mu_n3;
...
mu_n3 = (fbV-faV)*hn3V - (c_beta-c_alpha)*facV*hn3V + W*fbarriern3V + nDependentMisfitAC3 + heterMechAC3;

//compute K*nx
dealii::Tensor<1, dim, dealii::VectorizedArray<double> > Knx1, Knx2, Knx3;
...
modelResidualsList[4].scalarValueResidual = mu_n3; //constV(0.0);
modelResidualsList[5].scalarValueResidual = nDependentMisfitAC1;

modelResidualsList[2].scalarGradResidual = Knx1;
modelResidualsList[3].scalarGradResidual = Knx2;
modelResidualsList[4].scalarGradResidual = Knx3;
