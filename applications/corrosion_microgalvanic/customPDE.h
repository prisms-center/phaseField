...
public:
  ...
  void setInitialCondition(const dealii::Point<dim> &p, 
                           const unsigned int index, double 
                           & scalar_IC, dealii::Vector<double> 
                           & vector_IC);
...
private:
  ...
  void explicitEquationRHS(variableContainer<dim,degree,dealii::
                           VectorizedArray<double> > & variable_list,
                           dealii::Point<dim, 
  ...
  void nonExplicitEquationRHS(variableContainer<dim,degree,dealii::
                              VectorizedArray<double> > & variable_list,
			      dealii::Point<dim, 
                              dealii::VectorizedArray<double> > q_point_loc) 
                              const;
  ...
  void postProcessedFields(const variableContainer<dim,degree,dealii::
                           VectorizedArray<double> > & variable_list,
			   variableContainer<dim,degree,dealii::
                           VectorizedArray<double> > & pp_variable_list,
  ...
  double getNucleationProbability(variableValueContainer 
                                  variable_value, double dV) const;
  #endif
  ...
  double EcorrCathodic = userInputs.get_model_constant_double
                         ("EcorrCathodic");
  ...
  double cathodeThickness = userInputs.get_model_constant_double
                            ("cathodeThickness");
  double tStepStartForV = userInputs.get_model_constant_double
                          ("tStepStartForV");
