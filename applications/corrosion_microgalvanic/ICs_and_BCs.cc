...
void customPDE<dim, degree>::setInitialCondition(const dealii::Point<dim>& p, const unsigned int index, double& scalar_IC, dealii::Vector<double>& vector_IC)
{
    ...
    */
    ...
    double hyperTan = tanh((abs(posx - 1 * SYSTEM_WIDTH) - 1 * cathodeThickness)
        / deltaV);
    ...
    if (index == 0) { // order parameter for the anodic phase, nAnodic

        scalar_IC = orderParaAnodic;
    } else if (index == 2) { // order parameter for the cathodic phase, nCathodic

        scalar_IC = orderParaCathodic;
    } else if (index == 4) { // electrolyte domain parameter, psi

        scalar_IC = domainParaPsi;
    } else if (index == 6) { // electrolyte potential Phi

        scalar_IC = initialGuessForPhi;
    } else { // all other variable fields
