...
scalarvalueType c_alpha = (B2*(c-cbtmin*hV) + A2*calmin*hV)/(A2*hV+B2*(1.0-hV));
scalarvalueType c_beta = (A2*(c-calmin*(1.0-hV))+B2*cbtmin*(1.0-hV))/(A2*hV+B2*(1.0-hV));
...
scalargradType eqx_c = constV(-McV*userInputs.dtValue)*(cx + (c_alpha - c_beta)*hnV*nx);
//For order parameter (gamma is a variable order parameter mobility factor)
scalarvalueType eq_n = n-constV(userInputs.dtValue*MnV)*gamma*((fbV-faV)*hnV - (c_beta-c_alpha)*fbcV*hnV + W_barrier*fbarriernV);
