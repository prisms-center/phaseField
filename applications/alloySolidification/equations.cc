    ...
    dealii::Point<dim, dealii::VectorizedArray<double>> q_point_loc) const
{
    ...
    // Antitrapping term
    scalargradType j_at;
    j_at = constV(1.0 / 2.0 / sqrt(2.0)) * (constV(1.0) + constV(1.0 - k) * U) * (xi / u_phi) * normal;

    // grad_phi and grad_U dot product term
    scalarvalueType con_term = (constV((1.0 - k) / 2.0) / u_con / u_con * (phix[0] * (Dtilde * q_phi * Ux[0] + j_at[0]) + phix[1] * (Dtilde * q_phi * Ux[1] + j_at[1])));

    // Define required equations
    scalarvalueType eq_U = (U + constV(userInputs.dtValue) * (constV(1.0) + constV(1.0 - k) * U) * xi / constV(2.0) / u_phi / u_con - constV(userInputs.dtValue) * con_term);

    scalargradType eqx_U = (constV(-1.0) * constV(userInputs.dtValue) * (Dtilde * q_phi * Ux + j_at) / u_con);
    ...
    dealii::Point<dim, dealii::VectorizedArray<double>> q_point_loc) const
{
    ...
    scalarvalueType eq_xi = ((phi - constV(lamda) * (U + tep + constV(U_off)) * (constV(1.0) - phi * phi)) * (constV(1.0) - phi * phi));
