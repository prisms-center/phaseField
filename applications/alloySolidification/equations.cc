...
// coeffcient before U
scalarvalueType tau_U = (constV(1.0+k)/constV(2.0)-constV(1.0-k)*phi/constV(2.0));

// Antitrapping term
scalargradType j_at;
j_at= constV(1.0/(2.0*sqrt(2.0)))*(constV(1.0)+constV(1.0-k)*U)*(xi/tau_phi)*normal;
    
// grad_phi and grad_U dot product term
scalarvalueType val_term1 = constV(userInputs.dtValue)*(constV(1.0)+constV(1.0-k)*U)*xi/(constV(2.0)*tau_phi*tau_U);
scalarvalueType val_term2 = constV(userInputs.dtValue)*(constV((1.0-k)/2.0)/(tau_U*tau_U)*(phix[0]*(Dtilde*((constV(1.0)-phi)/constV(2.0))*Ux[0]+j_at[0])+phix[1]*(Dtilde*((constV(1.0)-phi)/constV(2.0))*Ux[1]+j_at[1])));

// Define required equations
scalarvalueType eq_U =  (U + val_term1 - val_term2);

scalargradType eqx_U =  (constV(-1.0)*constV(userInputs.dtValue)*(Dtilde*((constV(1.0)-phi)/constV(2.0))*Ux+j_at)/tau_U);
