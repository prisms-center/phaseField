...
for (unsigned int i=0; i<2*dim-1+dim/3; i++){
	  for (unsigned int j=0; j<2*dim-1+dim/3; j++){
		  CIJ_combined[i][j] = CIJ_list[0][i][j]*(constV(1.0)-h1strainV-h2strainV-h3strainV) + CIJ_list[1][i][j]*(h1strainV) + CIJ_list[2][i][j]*(h2strainV) + CIJ_list[3][i][j]*(h3strainV);
	  }
}
...
if (n_dependent_stiffness == true){
	...
	computeStress<dim>(CIJ_list[2]-CIJ_list[0], E2, S2);
	for (unsigned int i=0; i<dim; i++){
		for (unsigned int j=0; j<dim; j++){
	...
	computeStress<dim>(CIJ_list[3]-CIJ_list[0], E2, S2);
	for (unsigned int i=0; i<dim; i++){
		for (unsigned int j=0; j<dim; j++){
...
if (n_dependent_stiffness == true){
	...
	CIJ_combined = CIJ_list[0]*(constV(1.0)-h1strainV-h2strainV-h3strainV) + CIJ_list[1]*(h1strainV) + CIJ_list[2]*(h2strainV) + CIJ_list[3]*(h3strainV);
