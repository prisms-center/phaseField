...
#define h1strainV h1V //(3.0*phi*phi - 2.0*phi*phi*phi) //h1V //(h1V*h1V*h1V*h1V*h1V)
#define hn1strainV hn1V //(6.0*(phi-phi*phi)*phin) //hn1V //(5.0*h1V*h1V*h1V*h1V*hn1V)
...
for (unsigned int i=0; i<2*dim-1+dim/3; i++){
	  for (unsigned int j=0; j<2*dim-1+dim/3; j++){
		  CIJ_combined[i][j] = CIJ_list[0][i][j]*(constV(1.0)-h1strainV) + CIJ_list[1][i][j]*h1strainV;
	  }
}
...
if (n_dependent_stiffness == true){
	...
	CIJ_combined = CIJ_list[0]*(constV(1.0)-h1strainV);
	CIJ_combined += CIJ_list[1]*(h1strainV);
...
if (n_dependent_stiffness == true){
  for (unsigned int i=0; i<2*dim-1+dim/3; i++){
	  for (unsigned int j=0; j<2*dim-1+dim/3; j++){
		  CIJ_combined[i][j] = CIJ_list[0][i][j]*(constV(1.0)-h1V) + CIJ_list[1][i][j]*h1V;
	  }
  }
  computeStress<dim>(CIJ_combined, E2, S);
}
