...
void variableAttributeLoader::loadVariableAttributes(){
    ...
    set_dependencies_value_term_LHS(6, "n, psi, grad(psi), cM, grad(cM), cP, Phi, grad(Phi), change(Phi), irxn");
...
void customPDE<dim,degree>::nonExplicitEquationRHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
 ...
 // --- Getting the values and derivatives of the model variables ---
    ...
    scalarvalueType fpsiV = WV*psi*(psi*psi-constV(1.0)+constV(2.0)*gammaV*n*n);
    ...
    scalarvalueType rirxnV = constV(icorrV)*eterm/(constV(1.0)+constV(icorrV)*eterm/imax);
...
void customPDE<dim,degree>::equationLHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
    ...
    // --- Calculation of residual terms for irxnp (the derivative of irxn wrt to Phi)   ---
    //Overpotential
    scalarvalueType eta=VsV-EcorrV-Phi;
    //Maximum current prefactor
    scalarvalueType prefac=zMV*FarC/(constV(1.0)-VMV*cM);
    ...
    //scalarvalueType irxnp = (denom_i*icorrV*xip*eterm - numer_i*icorrV*xip*eterm/imax)/(denom_i*denom_i);
    scalarvalueType irxnp = icorrV*xip*eterm*(denom_i-numer_i/imax)/(denom_i*denom_i);
