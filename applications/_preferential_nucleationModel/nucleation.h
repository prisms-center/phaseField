...
double customPDE<dim,degree>::nucProb(double cValue, double dV, double dk1, double dk2) const
{
	// Calculate the nucleation rate
	double J=dk1*exp(-dk2/(std::max(cValue-calmin,1.0e-6)));
...
void customPDE<dim,degree>::getLocalNucleiList(std::vector<nucleus<dim>> &newnuclei) const
{
	...
	while (di != this->dofHandlersSet_nonconst[0]->end())
	{
		if (di->is_locally_owned()){
            ...
            //Obtaining average element concentration, element volume and element center
            double ele_vol = 0.0;
            double ele_av_conc = 0.0;
            std::vector<double> ele_center(dim,0.0);
            ...
            //Nucleation probability
            double Prob = 0.0;
            //Grain boundary in the x=0 plane
            if ((ele_center[0] > gbll) && (ele_center[0] < gbrl)){
            	//Grain boundary
            	Prob=nucProb(ele_av_conc,ele_vol,k1,k2_gb);
            } else {
                //Bulk
                Prob=nucProb(ele_av_conc,ele_vol,k1,k2_b);
            }
...
// =========================================================================================
//Periodically refines mesh within bounds of designated preferential nucleation area
// =========================================================================================
template <int dim, int degree>
void customPDE<dim,degree>::adaptiveRefineCriterion()
{
    ...
    QGaussLobatto<dim>  quadrature(degree+1);
    FEValues<dim> fe_values (*(this->FESet[this->userInputs.refine_criterion_fields[0]]), quadrature, update_values|update_quadrature_points);
    const unsigned int   num_quad_points = quadrature.size();
    std::vector<dealii::Point<dim> > q_point_list(num_quad_points);
    
    std::vector<double> errorOut(num_quad_points);
    
    typename DoFHandler<dim>::active_cell_iterator cell = this->dofHandlersSet_nonconst[this->userInputs.refine_criterion_fields[0]]->begin_active(), endc = this->dofHandlersSet_nonconst[this->userInputs.refine_criterion_fields[0]]->end();
    ...
    for (;cell!=endc; ++cell){
        if (cell->is_locally_owned()){
            ...
            for (unsigned int field_index=0; field_index<this->userInputs.refine_criterion_fields.size(); field_index++){
                fe_values.get_function_values(*(this->solutionSet[this->userInputs.refine_criterion_fields[field_index]]), errorOut);
            ...
            for (unsigned int q_point=0; q_point<num_quad_points; ++q_point){
                for (unsigned int field_index=0; field_index<this->userInputs.refine_criterion_fields.size(); field_index++){
                    bool cond_1 = (errorOutV[field_index][q_point]>this->userInputs.refine_window_min[field_index]) && (errorOutV[field_index][q_point]<this->userInputs.refine_window_max[field_index]);
            ...
            if ( (mark_refine && current_level < this->userInputs.max_refinement_level) ){
                cell->set_refine_flag();
            }
            else if (!mark_refine && current_level > this->userInputs.min_refinement_level) {
